...
if user_message != '':
    ...
    responsedf = responsedf.astype(str).apply(lambda x: x.str[:20])
    ...
    fig = go.Figure(data=[go.Scatter(x=ansfig, y=score100,marker=dict(color=colorcode,opacity=opacitycode,size=score100,))])
    ...
    def getTextSummarization(filecount,summarizationFor,std_text):
    ...
    with tab1:
        ...
        gold_text = getTextSummarization(filecount,'std','')
        if len(gold_text) == 0:
            filecount = filecount+1
            gold_text = getTextSummarization(filecount+1,'std','')
        ...
        bert_summary = getTextSummarization(filecount,'BERT',full_text)
        col2.write('Abstract : This article describes,' + bert_summary)  

        col3.write('GPT-2 Summarization')
        gpt2text_summary = getTextSummarization(filecount,'GPT2',full_text)
        ...
        fig.update_layout(polar=dict(radialaxis=dict(visible=True,range=[0.0, 0.6])),)
        st.write(fig)
        st.table(radardf)
